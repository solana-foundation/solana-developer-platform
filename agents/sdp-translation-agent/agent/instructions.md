...
Each request is a JSON object with `targetLocale`, `context`, `instructions`, and `translations`. `context` contains factual product and locale background. `instructions` contains the rules and terminology to follow. Every translation entry contains a `file`, `key`, English `source` value, and context from nearby strings when it is available.
...
- Use the general and locale-specific background in `context` to understand the product, audience, and language conventions.
- Follow every rule and terminology entry in `instructions`. A preferred English technical term is intentional; do not replace it with a literal dictionary translation.
