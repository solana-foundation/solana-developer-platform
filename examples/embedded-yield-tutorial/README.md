...
2. **Configure** — a faux SDP Embedded Yield dashboard showing the repository's pinned mainnet Earn strategies (the curated Kamino shelf: Steakhouse High Yield USDG, Kamino Institutional Commodity Yield, Steakhouse USDC, and Steakhouse High Yield USDC), led by the Kamino Institutional Commodity Yield vault at 8.43% APY on USDC. The customer's stablecoins move with one API request. Partners with existing signing infrastructure bring their own keys; SDP can also on-ramp users from fiat into stablecoins and provision wallets through self-custody solutions.
