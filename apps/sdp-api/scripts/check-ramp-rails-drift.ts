...
function summarizeList(title: string, values: readonly string[]): string[] {
  if (values.length === 0) return [];
  const visible = values.slice(0, MAX_LISTED_CHANGES);
  const lines = [`  - ${title}:`];
  for (const value of visible) {
    lines.push(`    - \`${value}\``);
  }
  if (values.length > visible.length) {
    lines.push(`    - ...and ${values.length - visible.length} more`);
  }
  return lines;
}
...
function buildComment(
  ...
  if (diffs.length === 0) {
    lines.push(
      "- Provider pair support is unchanged from the base branch, but the PR generated file is stale or unexpected."
    );
    return `${lines.join("\n")}\n`;
  }
  ...
  for (const diff of diffs) {
    lines.push(
      `- \`${diff.provider}\`: hash \`${diff.baseHash.slice(0, 12)}\` -> \`${diff.currentHash.slice(0, 12)}\`; on-ramp ${diff.base.onramp.length} -> ${diff.current.onramp.length} (+${diff.addedOnramp.length}/-${diff.removedOnramp.length}), off-ramp ${diff.base.offramp.length} -> ${diff.current.offramp.length} (+${diff.addedOfframp.length}/-${diff.removedOfframp.length})`
    );
  }

  lines.push("", "### Changes");
  for (const diff of diffs) {
    lines.push("", `#### ${diff.provider}`);
    lines.push(...summarizeList("Added on-ramp rails", diff.addedOnramp));
    lines.push(...summarizeList("Removed on-ramp rails", diff.removedOnramp));
    lines.push(...summarizeList("Added off-ramp rails", diff.addedOfframp));
    lines.push(...summarizeList("Removed off-ramp rails", diff.removedOfframp));
    if (
      diff.addedOnramp.length === 0 &&
      diff.removedOnramp.length === 0 &&
      diff.addedOfframp.length === 0 &&
      diff.removedOfframp.length === 0
    ) {
      lines.push("- Hash changed, but pair-level support is unchanged.");
    }
  }
