...
function buildComment(diffs: readonly ProviderDiff[]): string {
  const lines = [
    COMMENT_MARKER,
    "## Ramp Rails Drift Detected",
    "",
    "Live provider rail data differs from the generated ramp support committed in this PR.",
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (diffs.length > 0) {
      await writeFile(COMMENT_PATH, buildComment(diffs), "utf8");
