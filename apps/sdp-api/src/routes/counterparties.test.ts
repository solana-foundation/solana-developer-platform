...
describe("Counterparties Routes", () => {
  ...
  describe("GET /v1/counterparties/:counterpartyId/requirements", () => {
    ...
    it("collects the BVNK contact identity fields for a fresh counterparty", async () => {
      const created = await createCounterparty({ externalId: "requirements_bvnk_collect" });
    ...
    it("collects the legal name when the fresh BVNK counterparty is a business", async () => {
      const created = await createCounterparty({
        externalId: "requirements_bvnk_collect_business",
        entityType: "business",
      });
      expect(created.status).toBe(201);
      const counterparty = (await created.json()).data.counterparty;
