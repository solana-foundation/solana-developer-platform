 ...
 */
async function retireRequestApiKey(): Promise<void> {
  const now = new Date().toISOString();
  const past = new Date(Date.now() - 60_000).toISOString();
  await getDb(env)
    .prepare(
      "UPDATE api_keys SET status = 'deactivated', revoked_at = ?, expires_at = ? WHERE id = ?"
    )
    .bind(now, past, TEST_API_KEY.id)
    .run();
}

describe("Approved signer check replay after key retirement", () => {
  installPaymentsRouteTestHooks();

  it("executes an approved signer check after the requesting key was retired", async () => {
  ...
  it("still refuses replaying other operation types once the key is retired", async () => {
    await seedApproverSession();
    await seedWalletControlProfile({
      rules: [
        {
    ...
    expect(approvedResponse.status).toBe(200);
    const approvedBody = (await approvedResponse.json()) as ApprovedOperationBody;
    expect(approvedBody.data.approvalRequest.operation.status).toBe("failed");
