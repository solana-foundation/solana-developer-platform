...
describe("POST /internal/dashboard/custody/provider-credentials", () => {
  ...
  it.each([
    "active",
    "failed",
  ] as const)("creates a fresh Connection beside %s history instead of implicitly replacing it", async (historyStatus) => {
    const { app, token } = buildApp();
  ...
  ] as const)("admits a pending $source Connection beside the selected active Project Config", async ({
    source,
    body,
  }) => {
    if (source === "runtime") {
