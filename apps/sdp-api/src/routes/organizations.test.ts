...
describe("Organizations routes", () => {
  ...
  describe("GET /v1/organizations/:orgId", () => {
    ...
    it("returns internal error when organization tier is invalid in storage", async () => {
      await getDb(env).prepare("UPDATE organizations SET tier = ? WHERE id = ?")
    ...
    it("returns internal error when organization status is invalid in storage", async () => {
      await getDb(env).prepare("UPDATE organizations SET status = ? WHERE id = ?")
  ...
  describe("DELETE /v1/organizations/:orgId", () => {
    ...
    it("soft deletes organization", async () => {
      ...
      const org = await getDb(env).prepare("SELECT status FROM organizations WHERE id = ?")
        .bind(TEST_ORG.id)
        .first<{ status: string }>();
    ...
    it("revokes all API keys on delete", async () => {
      ...
      const keys = await getDb(env).prepare("SELECT status FROM api_keys WHERE organization_id = ?")
        .bind(TEST_ORG.id)
        .all<{ status: string }>();
