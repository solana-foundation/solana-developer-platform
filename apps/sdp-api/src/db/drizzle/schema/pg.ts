...
export const tokens = pgTable(
  "tokens",
  {
    ...
    isMintable: boolean("is_mintable").default(true),
    isFreezable: boolean("is_freezable").default(true),
    requiresAllowlist: boolean("requires_allowlist").default(false),
    status: text("status").notNull().default("pending"),
  ...
  (table) => ({
    ...
    mintUnique: uniqueIndex("tokens_mint_address_unique").on(table.mintAddress),
  })
);
...
export const frozenAccounts = pgTable(
  "frozen_accounts",
  {
    id: text("id").primaryKey(),
    tokenId: text("token_id")
    ...
    reason: text("reason"),
    frozenAt: text("frozen_at").notNull().default(sql`(now())`),
...
export const custodyConfigs = pgTable(
  "custody_configs",
  {
    id: text("id").primaryKey(),
    organizationId: text("organization_id")
