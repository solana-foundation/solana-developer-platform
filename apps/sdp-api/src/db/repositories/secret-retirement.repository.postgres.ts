...
function mapRow(row: Record<string, unknown>): SecretRetirementRow {
  return {
    id: row.id as string,
...
export async function insertSecretRetirement(
  exec: Pick<AppDb, "prepare">,
  input: RecordSecretRetirementInput
): Promise<void> {
  ...
  await exec
    .prepare(
      `INSERT INTO secret_retirements
         (id, organization_id, source_id, storage_backend, secret_ref, secret_version_ref,
          last_error, next_attempt_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, COALESCE(?, sdp_iso_now()))
       ON CONFLICT (secret_version_ref) DO UPDATE
         SET last_error = EXCLUDED.last_error,
             next_attempt_at = COALESCE(?, secret_retirements.next_attempt_at),
...
export async function deleteSecretRetirement(
  exec: Pick<AppDb, "prepare">,
  secretVersionRef: string
): Promise<boolean> {
  const row = await exec
    .prepare(
      `DELETE FROM secret_retirements
 ...
 */
export async function cancelUnclaimedSecretRetirement(
  exec: Pick<AppDb, "prepare">,
  secretVersionRef: string
): Promise<boolean> {
  const row = await exec
...
export function createPostgresSecretRetirementsRepository(db: AppDb): SecretRetirementsRepository {
  return {
    async recordRetirement(input: RecordSecretRetirementInput) {
      await insertSecretRetirement(db, input);
    },

    async deleteRetirementByVersionRef(secretVersionRef: string) {
      await deleteSecretRetirement(db, secretVersionRef);
    },

    async hasRetirement(secretVersionRef: string) {
      const row = await db
        .prepare("SELECT 1 FROM secret_retirements WHERE secret_version_ref = ?")
        .bind(secretVersionRef)
    ...
    async listDueRetirements(params) {
      const result = await db
        .prepare(
          `SELECT * FROM secret_retirements
    ...
    async deleteRetirement(id: string) {
      await db.prepare("DELETE FROM secret_retirements WHERE id = ?").bind(id).run();
    },
