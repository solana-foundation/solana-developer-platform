...
describe("EarnRepository (postgres)", () => {
  ...
  describe("provider wallets (earn_provider_wallets)", () => {
    ...
    /**
     * A custody wallet in some organization. `custody_wallets` has no
     * organization_id — ownership lives on the config — so the pair is seeded
     * together and both statuses are settable, because both gate ownership.
     */
      ...
      await db
        .prepare(
          `INSERT INTO custody_configs
           ...
           VALUES (?, ?, NULL, 'fireblocks', 'encrypted', 'sdp-custody-encryption-v1', ?)`
        )
        .bind(params.configId, params.organizationId, params.configStatus ?? "active")
        .run();
      await db
        .prepare(
          `INSERT INTO custody_wallets
             (id, custody_config_id, wallet_id, public_key, label, purpose, status)
           VALUES (?, ?, ?, ?, 'Treasury Ops', NULL, ?)`
        )
        .bind(
          params.walletId,
          params.configId,
          `provider-${params.walletId}`,
          `pk-${params.walletId}`,
          params.walletStatus ?? "active"
        )
        .run();
      return params.walletId;
    }

    describe("organizationOwnsCustodyWallet", () => {
      // This is the ONLY cross-tenant guard on a caller-supplied wallet id: no
      // foreign key can express org ownership, so these cases are the contract.
      it("accepts the organization's own active wallet", async () => {
        const walletId = await seedCustodyWallet({
          organizationId: TEST_ORG.id,
          configId: "ccfg_own",
          walletId: "cwlt_own",
        });
        await expect(
          repo.organizationOwnsCustodyWallet({
            organizationId: TEST_ORG.id,
      ...
      it("refuses another organization's wallet", async () => {
        await seedSiblingOrg();
        const walletId = await seedCustodyWallet({
          organizationId: OTHER_ORG.id,
          configId: "ccfg_sibling",
          walletId: "cwlt_sibling",
        });
        await expect(
          repo.organizationOwnsCustodyWallet({
            organizationId: TEST_ORG.id,
      ...
      it("refuses a wallet id that does not exist", async () => {
        await expect(
          repo.organizationOwnsCustodyWallet({
            organizationId: TEST_ORG.id,
      ...
      it("refuses a deactivated wallet — it cannot originate a transfer", async () => {
        const walletId = await seedCustodyWallet({
          organizationId: TEST_ORG.id,
          configId: "ccfg_inactive_wallet",
          walletId: "cwlt_inactive",
      ...
      it("refuses a wallet whose custody config is deactivated", async () => {
        const walletId = await seedCustodyWallet({
          organizationId: TEST_ORG.id,
          configId: "ccfg_inactive_cfg",
          walletId: "cwlt_cfg_inactive",
    ...
    describe("funding wallet (migration 0050)", () => {
      it("round-trips a non-null funding wallet through insert and read", async () => {
        const walletId = await seedCustodyWallet({
          organizationId: TEST_ORG.id,
          configId: "ccfg_funding",
          walletId: "cwlt_funding",
        });
        const inserted = await seedProviderWallet({ fundingWalletId: walletId });
        expect(inserted.funding_wallet_id).toBe(walletId);
        ...
        expect(fetched?.funding_wallet_id).toBe(walletId);
      });

      it("sets, repoints and clears the funding wallet", async () => {
        const first = await seedCustodyWallet({
          organizationId: TEST_ORG.id,
          configId: "ccfg_first",
          walletId: "cwlt_first",
        });
        const second = await seedCustodyWallet({
          organizationId: TEST_ORG.id,
          configId: "ccfg_second",
          walletId: "cwlt_second",
        });
        const row = await seedProviderWallet();
        expect(row.funding_wallet_id).toBeNull();

        await expect(
          repo.setProviderWalletFundingWallet({ id: row.id, fundingWalletId: first })
        ).resolves.toMatchObject({ funding_wallet_id: first });
        await expect(
          repo.setProviderWalletFundingWallet({ id: row.id, fundingWalletId: second })
        ).resolves.toMatchObject({ funding_wallet_id: second });
        await expect(
          repo.setProviderWalletFundingWallet({ id: row.id, fundingWalletId: null })
        ).resolves.toMatchObject({ funding_wallet_id: null });
      });

      it("returns null when no program row matches, so the caller can fail loudly", async () => {
        await expect(
          repo.setProviderWalletFundingWallet({
            id: "earn_provider_wallet_missing",
            fundingWalletId: null,
          })
        ).resolves.toBeNull();
      });

      it("clears the reference when the funding wallet is deleted, never cascading", async () => {
        // The whole point of ON DELETE SET NULL: a program holds real balances,
        // so losing its funding wallet must not lose the program.
        const walletId = await seedCustodyWallet({
          organizationId: TEST_ORG.id,
          configId: "ccfg_doomed",
          walletId: "cwlt_doomed",
        });
        const row = await seedProviderWallet({ fundingWalletId: walletId });

        await getDb(env).prepare("DELETE FROM custody_wallets WHERE id = ?").bind(walletId).run();

        const survived = await repo.getProviderWallet({
          organizationId: TEST_ORG.id,
          environment: "sandbox",
          provider: "ground",
        });
        expect(survived?.id).toBe(row.id);
        expect(survived?.funding_wallet_id).toBeNull();
      });
    });
