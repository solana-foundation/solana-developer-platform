...
BEGIN
  FOREACH tenant_table IN ARRAY ARRAY[
    ...
    'private_channel_wallet_revocations',
    'kyc_wallets',
...
-- underlying row-level security instead.
--
-- Applied by sweeping pg_class rather than naming views: the view set churns
-- (0068 dropped the four earn projections 0063 introduced), and a hardcoded
-- list would either fail on a dropped view or silently miss a new one. The
-- coverage test ratchets the invariant for every future view.
-- ---------------------------------------------------------------------------
DO $$
DECLARE
  view_name TEXT;
