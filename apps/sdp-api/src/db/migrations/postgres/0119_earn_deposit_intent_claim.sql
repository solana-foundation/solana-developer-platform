...
-- tabs quoted at different moments. swapSlippageBps is excluded with it; the
-- funding mint is included because it changes what leaves the wallet.
--
...
COMMENT ON COLUMN earn_movements.deposit_intent_fingerprint IS
    'Stable identity of the logical custody deposit intent (organization, project, environment, provider, vault, custody wallet, resolved asset identity, amount, swap source), excluding the quote-derived minSharesOut floor. Backs the cross-key claim: a different idempotency key submitting the same unchanged intent is answered with the still-open movement instead of starting a second sign/record/broadcast path.';
