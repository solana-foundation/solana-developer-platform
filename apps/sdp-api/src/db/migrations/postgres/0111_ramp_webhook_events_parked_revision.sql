-- Records WHICH application revision exhausted a ramp webhook event, so the
...
-- runtime with no revision identity) re-arms once and then carries the
...
COMMENT ON COLUMN ramp_webhook_events.parked_app_revision IS
  'Application revision (K_REVISION) that exhausted this event''s replay attempts. Compared on each replay pass: a mismatch re-arms the row.';
