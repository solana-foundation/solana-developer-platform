...
const CONFIGURED = {
  ...
  HELIUS_RINGS_DETERMINISTIC_KA_SEED: Buffer.alloc(32, 1).toString("base64"),
} as unknown as Env;
...
describe("resolveRingsGateway", () => {
  it("keeps the not-implemented gateway unless the adapter is selected", () => {
    for (const adapter of [undefined, "none", "http", "TS"]) {
      const gateway = resolveRingsGateway(
        { ...CONFIGURED, HELIUS_RINGS_ADAPTER: adapter } as Env,
        TENANT
      );
  ...
  it("builds the live gateway when the adapter is selected and configured", () => {
    expect(resolveRingsGateway(CONFIGURED, TENANT)).not.toBeInstanceOf(NotImplementedRingsGateway);
  });
  ...
  it.each([
    ...
    // Without it nothing can derive a key, so a deployment that selects the
    // adapter and omits it is misconfigured in the same way as a missing URL.
    ["HELIUS_RINGS_DETERMINISTIC_KA_SEED"],
  ])("names %s in the health detail when it is missing", async (variable) => {
    const gateway = resolveRingsGateway({ ...CONFIGURED, [variable]: undefined } as Env, TENANT);
