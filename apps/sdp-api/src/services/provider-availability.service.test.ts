...
describe("provider-availability.service", () => {
   ...
   * API and no credential — the same shape as Kamino. Pinned here because the
   * tempting mistake is to read the private npm token that fetches Veda's SDK
   * as a provider credential: that one is BUILD-time, never present in a running
   * deployment, and declaring it would make every environment report Veda
   * unconfigured while withdrawals still had to work.
   */
