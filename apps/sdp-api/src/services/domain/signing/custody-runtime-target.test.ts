...
describe("CustodyRuntimeTargets", () => {
  ...
  it.each([
    ["privy", true],
    ["turnkey", false],
  ] as const)("selects a completed Connection over a %s Config only when the provider matches", async (configProvider, expectedSelected) => {
    const config = await seedConfig({ provider: configProvider });
    const connection = await seedConnection();
    await setProjectDefault(config.id, null);
    const targets = new CustodyRuntimeTargets(getDb(env), env, new Map());

    await expect(
      targets.selectCompletedConnectionIfEligible({
        organizationId: ORGANIZATION_ID,
        projectId: PROJECT_ID,
        connectionId: connection.id,
      })
    ).resolves.toBe(expectedSelected);
    expect(await getProjectDefault()).toEqual({
      default_custody_config_id: config.id,
      default_custody_connection_id: expectedSelected ? connection.id : null,
    });
  });

  it("selects a first completed Connection without a Project target", async () => {
    const connection = await seedConnection();
    const targets = new CustodyRuntimeTargets(getDb(env), env, new Map());
  ...
  it("selects a completed Connection over a same-provider Organization fallback", async () => {
    const config = await seedConfig({ provider: "privy", projectId: null });
    const connection = await seedConnection();
    await setOrganizationDefault(config.id);
    const targets = new CustodyRuntimeTargets(getDb(env), env, new Map());
  ...
  it("does not auto-select over another active Connection", async () => {
    await seedConnection({ id: "cconn_existing", credentialId: "pcred_existing" });
    const candidate = await seedConnection({
      id: "cconn_candidate",
      credentialId: "pcred_candidate",
    });
    const targets = new CustodyRuntimeTargets(getDb(env), env, new Map());
    ...
    expect(await getProjectDefault()).toBeNull();
  });

  it("replaces a selected deactivated Connection with the completed candidate", async () => {
    const stale = await seedConnection({ id: "cconn_stale", credentialId: "pcred_stale" });
    await setProjectDefault(null, stale.id);
    await getDb(env)
      .prepare(
    ...
    expect(await getProjectDefault()).toEqual({
      default_custody_config_id: null,
      default_custody_connection_id: candidate.id,
    });
  });

  it("serializes Install Completion with a different-provider Config selection", async () => {
    const config = await seedConfig({ provider: "turnkey" });
    const connection = await seedConnection();
    const targets = new CustodyRuntimeTargets(getDb(env), env, new Map());
    ...
    await Promise.all([
      ...
      configStore.setDefaultConfig(ORGANIZATION_ID, PROJECT_ID, config.id),
    ]);
