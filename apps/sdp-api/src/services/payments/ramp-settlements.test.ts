...
describe("applyRampSettlementEvent", () => {
  ...
  it("writes the observed fiat pay-in to an on-ramp transfer from a settling event", async () => {
    await seedTransfer({
      id: "xfr_payin_settling",
      reference: "order_payin_settling",
    ...
    expect(await readTransfer("xfr_payin_settling")).toMatchObject({
      status: "settling",
      fiat_amount: "149.5",
      amount: "10",
    });
  ...
  it("writes the observed crypto amount to an off-ramp transfer from a settling event", async () => {
    await seedTransfer({
      id: "xfr_payout_settling",
      reference: "order_payout_settling",
      status: "awaiting_payment",
      type: "offramp",
      provider: "bvnk",
    });
    ...
    expect(await readTransfer("xfr_payout_settling")).toMatchObject({
      status: "settling",
      amount: "8.75",
      fiat_amount: "10",
    });
  ...
  it("allows a settling event to refresh an already settling transfer", async () => {
    await seedTransfer({
      id: "xfr_double_settling",
      reference: "order_double_settling",
      status: "settling",
      type: "onramp",
      provider: "bvnk",
    });

    const first = await applyRampSettlementEvent(env, {
      provider: "bvnk",
      kind: "settling",
      reference: "order_double_settling",
      receivedAmount: "99",
    });
    const second = await applyRampSettlementEvent(env, {
      provider: "bvnk",
      kind: "settling",
      reference: "order_double_settling",
      receivedAmount: "99.5",
    ...
    expect(await readTransfer("xfr_double_settling")).toMatchObject({
      status: "settling",
      fiat_amount: "99.5",
    });
  });

  it("reports no transition when the settling event targets a terminal transfer", async () => {
    await seedTransfer({
      id: "xfr_settling_final",
      reference: "order_settling_final",
      status: "completed",
      provider: "bvnk",
    });

    const result = await applyRampSettlementEvent(env, {
      provider: "bvnk",
      kind: "settling",
      reference: "order_settling_final",
    });
    ...
    expect(await readTransfer("xfr_settling_final")).toMatchObject({ status: "completed" });
  });

  it("reports a real transition for a settled transfer", async () => {
    await seedTransfer({
      id: "xfr_settled_applied",
      reference: "order_settled_applied",
      status: "settling",
      provider: "bvnk",
    });
