...
describe("reconcilePaymentRequest", () => {
  ...
  it("skips the chain check when the last one is inside the interval", async () => {
    findReferenceSpy.mockRejectedValue(new FindReferenceError("not found"));
    const request = await createRequest();

    const first = await reconcilePaymentRequest(env, request, {
      bestEffort: true,
      minChainCheckIntervalMs: 30_000,
    });
    const second = await reconcilePaymentRequest(env, request, {
      bestEffort: true,
      minChainCheckIntervalMs: 30_000,
    });
    ...
    expect(findReferenceSpy).toHaveBeenCalledTimes(1);
  });

  it("coalesces concurrent gated reconciles into a single chain check", async () => {
    findReferenceSpy.mockRejectedValue(new FindReferenceError("not found"));
    const request = await createRequest();
  ...
  it("rechecks a request once the interval has elapsed", async () => {
    findReferenceSpy.mockRejectedValue(new FindReferenceError("not found"));
    const request = await createRequest();

    await reconcilePaymentRequest(env, request, {
      bestEffort: true,
      minChainCheckIntervalMs: 0,
    });
    const second = await reconcilePaymentRequest(env, request, {
      bestEffort: true,
      minChainCheckIntervalMs: 0,
    ...
    expect(second.status).toBe("awaiting_payment");
    expect(findReferenceSpy).toHaveBeenCalledTimes(2);
  });

  it("does not gate reads that omit the interval", async () => {
    findReferenceSpy.mockRejectedValue(new FindReferenceError("not found"));
    const request = await createRequest();
  ...
  it("settles a first-read payment on a gated read", async () => {
    mockSettlementSucceeds();
    const request = await createRequest();

    const settled = await reconcilePaymentRequest(env, request, {
      bestEffort: true,
      minChainCheckIntervalMs: 30_000,
    });
  ...
  it("does not link a request to a conflicting transfer with the same signature", async () => {
    const request = await createRequest();
    const transfer = await createPaymentsRepository(
      env,
      createTenantScope({ organizationId: TEST_ORG.id, projectId: TEST_PROJECT_ID })
    ).createTransfer({
    ...
    if (!transfer) throw new Error("Expected conflicting transfer fixture creation to succeed");
    await getDb(env)
  ...
  it("returns an unresolved legacy request on best-effort reads without touching the chain", async () => {
  ...
  it("rejects an unresolved legacy request when current state is required", async () => {
    const request: PaymentRequestRow = {
      ...(await createRequest()),
      custody_wallet_id: null,
    };
