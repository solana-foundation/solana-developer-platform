...
import { RAMP_TRANSFERS_CRON } from "./ramp-transfers";
import {
...
vi.mock("./ramp-transfers", () => ({
  RAMP_TRANSFERS_CRON: "*/5 * * * *",
  runRampTransfersReconciliation: vi.fn(),
}));
...
describe("startCron", () => {
  ...
  it("schedules a task with PENDING_TRANSFERS_CRON when DISABLE_CRON is unset", () => {
    ...
    expect(scheduleMock.mock.calls[2][0]).toBe(RAMP_TRANSFERS_CRON);
