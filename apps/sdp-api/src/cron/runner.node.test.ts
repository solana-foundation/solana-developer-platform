...
// Touches the database and Redis; mocked so a scheduled tick can fire in tests.
vi.mock("./retention-purge", () => ({
  RETENTION_PURGE_CRON: "30 4 * * *",
...
describe("startCron", () => {
  ...
  // reconciler runs.
  //
  // The secret-retirement sweep and the retention purge are registered last (in that
  // order) and behind no flag at all, so both are in every count below too — including
  // the ones where asset profiles is off.
  ...
  it("schedules a task with PENDING_TRANSFERS_CRON when DISABLE_CRON is unset", () => {
    ...
    expect(scheduleMock.mock.calls[4][0]).toBe(RETENTION_PURGE_CRON);
  });
  ...
  it("schedules workflow executions by default, and its tick runs the engine", () => {
    ...
    (scheduleMock.mock.calls[2][1] as () => void)();
  ...
  it("still sweeps secret retirements when asset profiles is off", () => {
    ...
    // The retention purge is unflagged for the same reason: the tables accrue rows
    // regardless of which features are on, so cleanup must outlive them.
  ...
  it("schedules deposit + withdrawal reconcilers when private channels are enabled", () => {
    ...
    // withdrawals + secret retirements + retention purge (recurring stays off).
