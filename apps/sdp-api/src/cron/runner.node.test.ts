...
import { BVNK_ONRAMP_EXPIRY_CRON } from "./bvnk-onramp-expiry";
...
describe("startCron", () => {
  ...
  it("schedules a task with PENDING_TRANSFERS_CRON when DISABLE_CRON is unset", () => {
    ...
    expect(scheduleMock.mock.calls[1][0]).toBe(BVNK_ONRAMP_EXPIRY_CRON);
    expect(scheduleMock.mock.calls[2][0]).toBe(PENDING_TRANSFERS_CRON);
    expect(scheduleMock.mock.calls[3][0]).toBe(REVOKED_API_KEY_CACHE_CRON);
    expect(scheduleMock.mock.calls[4][0]).toBe(RECURRING_PAYMENTS_COLLECTION_CRON);
    ...
    expect(scheduleMock.mock.calls[10][0]).toBe(SECRET_RETIREMENTS_CRON);
  ...
  it("emits a sdp_cron_run proof-of-life event when a monitored tick runs", async () => {
    ...
    expect(logEvent).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        event: "sdp_cron_run",
        monitor: "sdp-api-retire-secrets",
        status: "ok",
  ...
  // The retirement queue is durable and only ever holds credentials that are ALREADY
  // orphaned, so the sweep runs behind no feature flag.
  it("still sweeps secret retirements when asset profiles is off", () => {
    ...
    expect(runSecretRetirements).toHaveBeenCalledWith({
