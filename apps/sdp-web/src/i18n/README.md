...
Product PRs add or change copy only in the English source catalogs. Do not update French in the same PR: the release workflow asks Eve to fill missing locale keys on `codex/release-main`. A dedicated translation-quality PR may change localized catalogs as long as it does not also introduce English source changes.
