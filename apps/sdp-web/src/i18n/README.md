...
Eve receives the locale rules in `.github/translation-guidance.json` plus the namespace and nearby approved translations for every new string. Put descriptive product, audience, and language facts under each `context` object. Put imperative style rules under `instructions`, and glossary entries under `terminology`; these are sent to Eve as separate request fields. `forbiddenTerms` is only used by deterministic catalog validation and is not part of the model prompt. In French, established product terms such as `Token` and `Wallet` stay in English; literal `jeton` and `portefeuille` variants are rejected by catalog validation.
