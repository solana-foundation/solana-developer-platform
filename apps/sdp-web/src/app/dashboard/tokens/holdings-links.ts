 ...
 *
 * The transactions table filters by token, so a per-token history needs no
...
export function tokenActivityHref(mint: string): string {
  const token = mint.trim().slice(0, ASSET_FILTER_MAX_LENGTH);
  if (!token) {
    return "/dashboard/payments/transactions";
  }
  return `/dashboard/payments/transactions?token=${encodeURIComponent(token)}`;
}
