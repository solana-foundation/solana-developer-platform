...
export function AddWalletDialog({
  ...
  // The form is mounted only while the dialog is open, so a half-typed name
  // leaves with it and the next open starts clean. Clearing the field by hand
  // on close would show the old value for a frame on the way out.
  return (
    <Modal
      isOpen={isOpen}
    ...
    >
      <AddWalletForm {...fields} onClose={onClose} pending={pending} run={run} t={t} />
    </Modal>
  );
}
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 p-6" data-custody-add-wallet-form>
      ...
      <div className="space-y-2">
        <p className="text-sm font-medium text-primary">{t("DashboardCustody.connectionColumn")}</p>
      ...
      <div className="space-y-2">
        <Label htmlFor="custody-add-wallet-label">{t("DashboardCustody.addWalletNameLabel")}</Label>
        <Input
