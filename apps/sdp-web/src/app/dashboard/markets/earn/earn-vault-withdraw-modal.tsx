...
function derivedMinAmountOut(
  ...
  return floorForTolerance(quote.preview.assetsOut, quote.preview.assetDecimals, toleranceBps);
}
...
export function EarnVaultWithdrawModal({
  ...
  const quoteKey =
    slippagePolicy !== null && sharesValidation.kind === "valid"
      ...
      : null;
  const quote = useDebouncedVaultQuote<EarnVaultWithdrawalPreview>(
    quoteKey,
    (signal) =>
      fetchEarnVaultWithdrawalPreview({ positionId: position.id, shares: quoteKey ?? "" }, signal),
    quoteRefreshKey
  );
    ...
    const result = await createEarnVaultWithdrawal(
      {
        positionId: position.id,
        shares,
        ...(minAmountOut === undefined ? {} : { minAmountOut }),
      },
      resolvedKey.key
    );
