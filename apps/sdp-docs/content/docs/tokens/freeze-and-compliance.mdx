...
`accountAddress` accepts a holder wallet or its token account — SDP derives the associated token account when needed. Reuse the same `Idempotency-Key` when retrying the exact request so a lost response replays the original action instead of submitting it twice.
