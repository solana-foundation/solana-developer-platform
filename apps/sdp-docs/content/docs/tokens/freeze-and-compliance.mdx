...
`accountAddress` accepts a holder wallet or its token account — SDP derives the associated token account when needed. Reuse the same `Idempotency-Key` when retrying the exact request so a lost response replays the original action instead of submitting it twice. Each new freeze is a new operation and gets a fresh key; replaying the old key with different details returns a conflict.
...
The prompt uses the only matching authority wallet automatically. If several custody records control the same authority, select the signer before confirming. The Unpause button uses the same selection rules.
...
curl -X POST https://api.solana.com/v1/issuance/tokens/tok_abc123/unpause \
  -H "Authorization: Bearer sk_test_..." \
  -H "Content-Type: application/json" \
  -H "Idempotency-Key: unpause-001" \
  -d '{"signingCustodyWalletId":"cwlt_example"}'
```

`signingCustodyWalletId` is optional when exactly one in-scope custody record matches the current pause authority; send `{}` to use that resolution. Multiple matches without a selector return `409`. An explicit choice must control the live authority and pass authorization and runtime checks. Reusing an idempotency key with a different wallet returns `409`.
