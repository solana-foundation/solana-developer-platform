...
On the token detail page, go to the **Compliance** tab. Click **Freeze account**, enter the holder's wallet address and an optional reason, then confirm.

To lift the freeze, find the account in the frozen list and click **Unfreeze**.
...
On the token detail page, click **Pause token** in the admin controls panel and confirm.

While paused, the token header shows a `Paused` badge and all supply and transfer operations are blocked. Click **Unpause** to resume.
