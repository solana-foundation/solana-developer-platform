...
```bash
curl -X POST https://sdp-api-production.solana.workers.dev/v1/issuance/tokens/tok_abc123/authority \
  -H "Authorization: Bearer sk_test_..." \
...
```bash
curl -X POST https://sdp-api-production.solana.workers.dev/v1/issuance/tokens/tok_abc123/authority/prepare \
  -H "Authorization: Bearer sk_test_..." \
  -H "Content-Type: application/json" \
  -d '{
    "authority": {
