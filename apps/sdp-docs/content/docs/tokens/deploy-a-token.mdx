...
Click **Deploy**. A confirmation modal shows the token details — review them, then click **Confirm deploy**.
...
The token status badge changes from `Pending` to `Active`. The detail page now shows the mint address and onchain supply (initially 0).
