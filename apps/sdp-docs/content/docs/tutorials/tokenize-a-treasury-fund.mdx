...
Together they encode Section 3(c)(7) qualified-purchaser admission into the token's mainnet specification, not an application-layer policy a compliance officer has to discover from your code. On devnet, Mosaic deactivates the on-chain frozen-default mechanism so the reader can exercise the SDP allowlist pre-flight without on-chain friction; the pre-flight still returns HTTP 403 `NOT_ON_TOKEN_ALLOWLIST` for non-allowlisted destinations, and Sections 5 and 7 exercise that layer.

SDP does not expose Token-2022 transfer hooks. The permanent-delegate + frozen-by-default pair covers the enforcement surface a hook would have given you. Unauthorized destinations are rejected at mint and transfer time, and tokens already in flight remain reachable by the delegate for compliance seizure.
...
The `overrides.extensions.<name>` nesting matters. A top-level `extensions` block returns HTTP 201 with the config silently dropped, and the multiplier never reaches the deployed mint.
...
The header is how you make network-level retries safe for irreversible operations like deploy, mint, burn, seize, and force-burn. Extend the `SdpClient` from Section 2:
...
| `permanentDelegate` | custody wallet |
...
**Off-chain pre-flight** runs on every state-mutating call, on every network. Mint to a non-allowlisted address and SDP rejects with `NOT_ON_TOKEN_ALLOWLIST` before signing any transaction.

**On-chain enforcement** runs on `mainnet-beta` only. The on-chain ABL list activates alongside Token-2022's default-frozen account state, catching anything that bypasses SDP. On devnet, the token deploys with `ablListAddress: null` and ATAs open `initialized`.
...
```js
await client.post(`/v1/issuance/tokens/${tokenId}/allowlist`, {
...
### Screen addresses before allowlisting
...
```js
await client.post("/v1/compliance/address-screenings", {
  address: holderAddress,
  network: "solana",
...
As of 2026-05-22, Chainalysis credentials are broken in production sandbox and the provider returns `status: "error"`. Range and Elliptic are the operational providers in this tutorial.
...
```js
await adminClient.post(`/v1/issuance/tokens/${tokenId}/freeze`, {
...
- `POST /mint/prepare` against the same body surfaces the underlying Token-2022 failure in its simulation output, the same diagnostic surface Section 4 used for unfunded-fee-payer deploys.
...
**Seize** and **force-burn** are the escalation tier. They force-transfer or force-burn tokens through the permanent delegate authority, used only under court order. Section 7 walks the worked example.

### Configure authority and name the seam
...
```js
await adminClient.post(`/v1/issuance/tokens/${tokenId}/authority`, {
...
const subscription = await sdp.post(
  ...
  { idempotencyKey: idempotencyKey("mint-subscription") }
);
...
Capture `tokenAccount` (the ATA) into your records. Section 7's compliance demo addresses the ATA directly when it freezes the holder.
...
const dividend = await sdp.post(
  ...
  { idempotencyKey: idempotencyKey("mint-dividend") }
);
```
...
Pull the transactions history to confirm both records landed with their memos.

```javascript
const { data } = await sdp.get(
  `/v1/issuance/tokens/${tokenId}/transactions`
);
console.log(`Records: ${data.length}`);
data.forEach(tx => console.log(`  ${tx.type.padEnd(8)} ${tx.params.amount ?? "-"}  ${tx.params.memo ?? ""}`));
```

The response is `{ data: [...], meta: { total, page, pageSize, hasMore, requestId } }`. `data` is an array of every state-changing transaction this token has produced: `mint`, `burn`, `freeze`, `seize`, `force-burn`, `unfreeze`. Each record carries its memo verbatim, the on-chain signature, the destination address, the amount, and the `createdAt` timestamp. This is the artifact your compliance officer reads.

Section 7 walks the compliance demo that exercises the failure surface: subscription mints succeed, the holder gets flagged by your screening provider, you freeze the holder's ATA, and the next mint attempt returns an opaque 500 that prepare-as-diagnostic resolves.
...
try {
  await sdp.post(
    ...
    { idempotencyKey: idempotencyKey("mint-blocked") }
  );
} catch (err) {
...
const resumed = await sdp.post(
  ...
  { idempotencyKey: idempotencyKey("mint-dividend-aug") }
);
...
| Non-allowlisted mint enforcement | Off-chain pre-flight only (HTTP 403) | Off-chain pre-flight plus on-chain ABL rejection |
...
Mainnet enforcement is stronger than what you rehearsed in sandbox. Section 5's hybrid-enforcement framing was deliberate: sandbox proves the off-chain pre-flight, and mainnet adds the on-chain layer that catches anything the API surface might miss.
...
- [ ] **Screening fresh.** The address screening providers (Range, Elliptic) returned a clean result on the holder address within the freshness window your compliance policy defines.
