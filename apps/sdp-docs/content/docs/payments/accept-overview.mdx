...
4. Your reconciliation worker (see [Indexing and reconciliation](/docs/payments/accept-indexing)) periodically lists recent inbound transfers — **without** a `wallet`/`walletAddress` filter (the wallet-scoped path uses recent signature history and does not honor `from`/`to`) — and matches each transfer's `destination` to an open order in your `orders` table.
