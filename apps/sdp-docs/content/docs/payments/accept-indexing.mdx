...
- Persist the high-water mark (`lastSeenIso`) after the pass completes so re-tries pick up at the same window.
- Allow a small overlap (re-query the last few seconds on each tick) to avoid missing transfers that arrive at the boundary.
