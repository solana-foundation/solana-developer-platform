...
| `from`, `to` | ISO 8601 datetime | Time-window filter on `createdAt`. **Honored only on the org-scoped path (no `wallet`/`walletAddress` filter).** On the wallet-scoped path the server replays the most recent on-chain signature history (~200 entries) and ignores `from`/`to` for the observed/confirmed branch. Use with offset (`2026-05-14T00:00:00Z`). |
