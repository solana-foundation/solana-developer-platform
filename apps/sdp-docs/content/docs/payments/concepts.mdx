...
Payments transfer **SPL tokens** — stablecoins, tokenized assets, or any mint your project supports. Amounts are always expressed in the **smallest unit**: for a token with 6 decimals, `"1000000"` means 1.0 token. Decimal handling is the caller's responsibility on the way in, and SDP returns both raw `amount` and human-readable `uiAmount` on balance and transfer reads.

The `token` field on a transfer accepts either the SDP-internal token identifier or the on-chain mint address.
...
| `token`, `amount` | What moved and how much (raw smallest unit). |
