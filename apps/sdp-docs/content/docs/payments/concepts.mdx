...
You provision custody wallets up front (see [Set Up Wallets](/docs/guides/setup-wallets)) and reference them by the `walletId` returned from `GET /v1/wallets` (for example `privy_wallet_123` — the form depends on the custody signing provider). Transfer endpoints expect the **source** as that `walletId` — the wallet is resolved server-side from the caller's custody-wallet list — and the **destination** as the on-chain Solana address of the recipient. Ramp quote endpoints accept the same `walletId` (or a raw Solana address) for `sourceWallet` / `destinationWallet` — see [Ramp providers](/docs/payments/ramps-providers) for which of the seven integrated providers support which direction.
