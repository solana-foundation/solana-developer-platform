...
SDP handles token transfers through its payments API. A basic payment moves tokens from a source account to a destination address — Solana addresses on the recipient side, and either an SDP-custody wallet or an externally-controlled key on the sender side.
