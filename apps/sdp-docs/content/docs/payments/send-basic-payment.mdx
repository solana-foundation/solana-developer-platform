...
- An API key with both `payments:write` and `wallets:read` permissions on the source wallet — both endpoints (`POST /v1/payments/transfers` and `POST /v1/payments/transfers/prepare`) are gated by that pair.
