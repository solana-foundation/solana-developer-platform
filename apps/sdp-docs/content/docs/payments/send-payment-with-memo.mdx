...
curl -X POST https://api.solana.com/v1/payments/transfers \
  ...
  -H "Idempotency-Key: payment-memo-4837" \
  -d '{
...
const response = await fetch(
  "https://api.solana.com/v1/payments/transfers",
  {
    ...
    headers: {
      ...
      "Idempotency-Key": "payment-memo-4837",
    },
    body: JSON.stringify({
      sourceCustodyWalletId: "cwlt_123",
      destination: "7xKXz...9fGh",
...
HttpRequest request = HttpRequest.newBuilder()
    ...
    .header("Idempotency-Key", "payment-memo-4837")
...
Reuse the same `Idempotency-Key` for retries of that exact request so a lost response replays the original transfer instead of creating a second one. A new payment — different amount or destination — is a new operation and needs a fresh key; only an exact retry reuses one.
