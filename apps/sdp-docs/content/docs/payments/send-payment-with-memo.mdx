...
SDP's `Prepare` endpoint accepts a `referenceAddress` field so you can attach a Solana Pay reference when building a transfer client-side. Use [Accept overview](/docs/payments/accept-overview) and [Indexing and reconciliation](/docs/payments/accept-indexing) for the inbound side.
