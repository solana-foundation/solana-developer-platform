...
| BVNK | ✓ | ✓ | ✓ | `manual_instructions` | Hawk auth. Compliance party details are built from the BVNK contact, not passed on the request. |
...
Counterparties no longer store identity or email fields. Existing Lightspark customers continue to resolve from provider data, but first-time customer onboarding currently fails with a clear error until just-in-time identity collection is connected.
...
BVNK is a travel-rule / compliance-sharing provider and requires party details. The requirements step collects the counterparty's identity once — first and last name for individuals, legal name for companies — and creates a BVNK contact, with no BVNK customer, KYC, or agreement flow.

BVNK quotes always return `deliveryMode: "manual_instructions"`. On-ramps fund a BVNK virtual funding wallet via a per-transfer payment rule declaring the counterparty a `THIRD_PARTY` entity. Off-ramps sell the crypto into the counterparty's BVNK settlement balance: deposit crypto, and fiat is credited to that balance. Bank withdrawal is not available yet.
...
| Travel-rule / compliance party metadata | BVNK — party identity declared via the collected contact. |
