...
| Hercle | ✓ | ✓ | ✓ | `manual_instructions` | Signed Key v1 (per-request HMAC). Business counterparties only; EUR ↔ USDC (Solana) launch corridor; KYB verification via a hosted link. |
...
Hercle serves **business counterparties only**, registered in Switzerland or the EEA; each counterparty becomes a regulated Hercle sub-account selected via the `on-behalf-of` header on one platform credential. Requirements flow: the first POST collects the company registration number and provisions the account, then a hosted KYB verification link is returned (`customer_verification_required`); quotes are refused until the verification verdict lands (`ready`). Launch corridor is EUR ↔ `usdc.solana`, both directions, `deliveryMode: "manual_instructions"` — on-ramp returns wire instructions for a Hercle-issued account, off-ramp returns a Solana deposit address.
