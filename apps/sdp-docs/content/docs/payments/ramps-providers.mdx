...
BVNK is a travel-rule / compliance-sharing provider and requires party details such as name and address. The requirements step therefore collects the counterparty's identity once — first name and last name for individuals, legal name for companies — and creates a BVNK contact for it. There is no BVNK customer, KYC, or agreement flow: wallets are merchant-owned, and payment rules and channels declare the counterparty as a `THIRD_PARTY` entity, referencing the contact via `complianceDetails.contactId`.
