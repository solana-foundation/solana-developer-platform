...
description: MoonPay, Lightspark, BVNK, MoneyGram, Coinbase, Mural, Stripe, and Hercle — capability matrix, configuration, and per-provider notes.
---

SDP currently integrates eight ramp providers. They are configured at the organization level via environment variables on the SDP API deployment; the same `provider` string on `POST /v1/payments/ramps/onramp/quote` and `POST /v1/payments/ramps/offramp/quote` selects between them.
...
| Hercle | ✓ | ✗ | ✓ | `manual_instructions` | Signed Key v1 (per-request HMAC). Business counterparties only; first-party fiat only; EUR, USD and GBP → USDC, USDT and SOL (Solana); KYB verification via a hosted link. Onramp only. |
...
MoonPay normalizes provider currency codes internally — SDP accepts the canonical asset rails such as `usdc.solana` and maps them to MoonPay's `usdc_sol` code.
...
Lightspark is integrated against the Grid API (`https://api.lightspark.com/grid/2025-10-13`); the sandbox/production choice follows the [per-request rule](#sandbox-vs-production-selection). `destinationCustodyWalletId` / `sourceCustodyWalletId` are your own custody wallet, same as every other provider — Lightspark's customer id and payout account are resolved server-side from the counterparty's Provider Requirements, not passed on the quote request.
...
| Variable | Required | Notes |
| --- | --- | --- |
| `HERCLE_CLIENT_ID` | conditional | Production client id (`hpk_live_…`). Required when the request resolves to production. |
| `HERCLE_CLIENT_SECRET` | conditional | Production Signed Key secret; every request carries an HMAC-SHA256 signature over `ts + METHOD + pathWithQuery + rawBody`. |
| `HERCLE_API_BASE_URL` | conditional | Production partner API base URL, provided at onboarding. |
| `HERCLE_WEBHOOK_PUBLIC_KEY` | conditional | ECDSA P-256 public key (PEM) verifying `X-Signature` over `X-Timestamp.rawBody` on inbound webhooks. |
| `HERCLE_SANDBOX_CLIENT_ID` | conditional | Sandbox client id (`hpk_test_…`). Required when the request resolves to sandbox. |
| `HERCLE_SANDBOX_CLIENT_SECRET` | conditional | Sandbox Signed Key secret. |
| `HERCLE_SANDBOX_API_BASE_URL` | conditional | Sandbox partner API base URL. |
| `HERCLE_SANDBOX_WEBHOOK_PUBLIC_KEY` | conditional | Sandbox webhook public key (PEM). |

Hercle serves **business counterparties only**, registered in Switzerland or the EEA; each counterparty becomes a regulated Hercle sub-account selected via the `on-behalf-of` header on one platform credential. The counterparty is Hercle's customer — the business that holds the fiat relationship and passes KYB once; the organization's custody wallet is the crypto side of every ramp and is never onboarded with Hercle. Fiat moves **first-party to that business**: it funds an on-ramp from its own bank account, and a wire from anyone else is returned by the bank and fails the order (`THIRD_PARTY_PAYER`). An organization ramping its own funds simply creates a counterparty for itself. **Onramp only**: like Mural, the rail declares no off-ramp corridor and `createOfframpQuote` refuses.

Requirements flow: the first POST collects the company registration number, the registered address and the business's acceptance of Hercle's terms, provisions the sub-account and returns a hosted KYB verification link (`customer_verification_required`); quotes are refused until the business is verified (`ready`). Provider state is a single `counterparty_provider_accounts` row (the `customer_link`); the collected data goes to Hercle and is never stored. Corridors are EUR, USD and GBP → `usdc.solana`, `usdt.solana` and `sol.solana`, `deliveryMode: "manual_instructions"` — the on-ramp returns wire instructions for the business's own dedicated account in the chosen currency at Hercle's bank together with `payerAccountHolder`, the name the wire must come from.

## Provider gating

Even when all eight providers' env vars are populated, an org can be restricted to a subset. This is [**Provider Availability**](/docs/reference/provider-onboarding): a provider must be configured for the deployment and either generally available or manually activated for the organization. SDP checks this on every ramp call — if the org lacks availability for the requested provider, the call returns an error. To activate or change provider availability for a deployment, see [Provider onboarding](/docs/reference/provider-onboarding).
