...
| Hercle | ✓ | ✗ | ✓ | `manual_instructions` | Signed Key v1 (per-request HMAC). Business counterparties only; first-party fiat only; EUR → USDC (Solana) launch corridor; KYB verification via a hosted link. Onramp only. |
