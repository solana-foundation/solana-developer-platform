...
| `bvnkCompliance` | no | BVNK-only: object of the form `{ "partyDetails": [...] }` carrying compliance party records. Omit the field entirely for `moonpay` and `lightspark`. See [Ramp providers](/docs/payments/ramps-providers#bvnk). |
