...
| `destinationWallet` | yes | SDP wallet ID or on-chain Solana address. |
...
| `kycReference` | no | Up to 128 chars; identifies the end-user across your KYC system. Some providers use this to short-circuit re-verification. |
| `redirectUrl` | no | Valid URL the provider sends the user to on completion. |
| `bvnkCompliance` | no | BVNK-only: party-detail array required for compliance. See [Ramp providers](/docs/payments/ramps-providers#bvnk). |
...
Offramp uses `sourceWallet` (the wallet you're cashing out from) and `cryptoAmount` (the crypto amount to convert). All other fields mirror the onramp request.
...
- **Compliance attachment** — only BVNK accepts the `bvnkCompliance` party-detail field today.
