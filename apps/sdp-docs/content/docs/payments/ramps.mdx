...
Every quote is tied to a **Counterparty** and a custody wallet you control. Providers that need a party identity (such as BVNK) collect the counterparty's identity once and resolve it to a provider-side contact. For per-provider configuration, see [Ramp providers](/docs/payments/ramps-providers).

## Prerequisites

1. **A Counterparty.** Create one with `POST /v1/counterparties` before requesting a quote.
2. **Provider Requirements satisfied**, if the provider needs them. Some providers require identity or bank details collected against the counterparty before they will price or accept a quote. Check `GET /v1/counterparties/{counterpartyId}/requirements?provider=…&direction=…` and submit collected fields with `POST` to the same path until `status` reads `ready` — BVNK moves through `collect` → `provisioning` → `ready`.
...
| `counterpartyId` | yes | An existing Counterparty. Provider-side contact and payout-account state is resolved server-side from it, not passed on the request — for BVNK, the `customer_link` provider-account row stores the BVNK contact id. Counterparties do not store identity or email fields. |
| `destinationCustodyWalletId` (onramp) / `sourceCustodyWalletId` (offramp) | yes | A custody wallet you control — its `id` (the `id` returned by `GET /v1/wallets`), resolved against your API key's wallet scope. Same shape for every provider. |
| `assetRail` | yes | Canonical SDP crypto asset rail (`usdc.solana`, `usdt.solana`, `sol.solana`, …). |
