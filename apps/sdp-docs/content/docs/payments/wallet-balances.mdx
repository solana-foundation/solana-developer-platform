...
`GET /v1/payments/wallets/{walletId}/balances` returns every SPL token balance held by a custody wallet — raw amount, human-readable amount, decimals, and optional USD valuation when a price source is configured.
