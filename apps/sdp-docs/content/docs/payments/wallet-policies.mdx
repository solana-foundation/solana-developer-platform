...
Amount fields are decimal strings in the **smallest unit** of whatever token the limit applies to. Today the limit is single-currency per wallet — if you need multi-token limits, partition into separate wallets.
