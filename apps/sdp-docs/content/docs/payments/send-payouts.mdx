...
2. **Transfer-record uniqueness** — the on-chain `signature` is `UNIQUE` across the SDP transfers table. Two requests that produce the same signed transaction collapse to one transfer row.
...
async function payoutRecipient(row: PayoutRow): Promise<string> {
  ...
  await row.persistTransferId(data.id); // persist BEFORE returning
  return data.id;
}
```
</Tab>
