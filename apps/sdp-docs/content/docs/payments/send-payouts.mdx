...
<Tab value="curl">
```bash
curl -X POST https://sdp-api-production.solana.workers.dev/v1/payments/transfers \
...
async function payoutRecipient(row: PayoutRow): Promise<string> {
  ...
  const response = await fetch(
    "https://sdp-api-production.solana.workers.dev/v1/payments/transfers",
    {
      method: "POST",
      headers: {
...
HttpRequest request = HttpRequest.newBuilder()
    .uri(URI.create("https://sdp-api-production.solana.workers.dev/v1/payments/transfers"))
...
<Tab value="curl">
```bash
curl "https://sdp-api-production.solana.workers.dev/v1/payments/transfers?direction=outbound&from=2026-05-14T00:00:00Z&to=2026-05-14T23:59:59Z&pageSize=100" \
  -H "Authorization: Bearer sk_test_..."
```
</Tab>
<Tab value="TypeScript">
...
async function* listOutboundTransfers(from: string, to: string) {
  let page = 1;
  while (true) {
    const url = new URL("https://sdp-api-production.solana.workers.dev/v1/payments/transfers");
