...
| `finalized` | The slot containing the transaction is locked in by the network — finality at the Solana cluster's strongest guarantee. `signature`, `slot`, and `blockTime` are all populated. Safe to treat as irreversible. |
