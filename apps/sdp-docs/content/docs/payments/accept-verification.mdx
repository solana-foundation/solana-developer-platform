...
| Status | Meaning |
| --- | --- |
| `pending` | SDP has accepted the request (or detected an inbound) but has not yet observed the transaction on the network. No `signature` yet. |
| `processing` | The transaction has been submitted; SDP is waiting for cluster confirmation. `signature` is now set. |
| `confirmed` | A supermajority of the cluster has voted on the transaction. The transfer is durable but not yet at maximum finality. `slot` and `blockTime` are populated. |
| `finalized` | The slot containing the transaction is locked in by the network — finality at the Solana cluster's strongest guarantee. Safe to treat as irreversible. |
...
A transfer moves forward through `pending → processing → confirmed → finalized`. Any of those can transition to `failed` instead; `failed` is terminal.
