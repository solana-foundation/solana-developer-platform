...
After [creating a token](/docs/guides/create-a-token), deploy it on-chain. Deployment creates the mint account on Solana and assigns authorities to your configured wallets.
...
// data.token.mintAddress — the on-chain address
```
</Tab>
<Tab value="Java">
```java
...
The token status changes from `pending` to `active`, and `mintAddress` is populated with the on-chain address. No request body is needed.
