...
<Tab value="curl">
```bash
curl -X POST /v1/issuance/tokens/tok_abc123/deploy \
...
const response = await fetch(
  "/v1/issuance/tokens/tok_abc123/deploy",
  {
    method: "POST",
    headers: {
...
HttpRequest request = HttpRequest.newBuilder()
    .uri(URI.create("/v1/issuance/tokens/tok_abc123/deploy"))
...
<Tab value="curl">
```bash
curl -X POST /v1/issuance/tokens/tok_abc123/deploy/prepare \
  -H "Authorization: Bearer sk_test_..."
```
</Tab>
<Tab value="TypeScript">
```typescript
const response = await fetch(
  "/v1/issuance/tokens/tok_abc123/deploy/prepare",
  {
    method: "POST",
...
```java
HttpRequest request = HttpRequest.newBuilder()
    .uri(URI.create("/v1/issuance/tokens/tok_abc123/deploy/prepare"))
    .header("Authorization", "Bearer sk_test_...")
    .POST(HttpRequest.BodyPublishers.noBody())
    .build();
```
