...
<Tab value="curl">
```bash
curl /v1/issuance/templates \
  -H "Authorization: Bearer sk_test_..."
```
</Tab>
<Tab value="TypeScript">
```typescript
const response = await fetch("/v1/issuance/templates", {
...
HttpRequest request = HttpRequest.newBuilder()
    .uri(URI.create("/v1/issuance/templates"))
...
<Tab value="curl">
```bash
curl -X POST /v1/issuance/tokens \
  -H "Authorization: Bearer sk_test_..." \
...
<Tab value="TypeScript">
```typescript
const response = await fetch("/v1/issuance/tokens", {
  method: "POST",
  headers: {
...
```java
HttpRequest request = HttpRequest.newBuilder()
    .uri(URI.create("/v1/issuance/tokens"))
    .header("Authorization", "Bearer sk_test_...")
