...
<Tab value="curl">
```bash
curl -X POST /v1/wallets/initialize \
...
```typescript
const response = await fetch("/v1/wallets/initialize", {
  method: "POST",
  headers: {
...
HttpRequest request = HttpRequest.newBuilder()
    .uri(URI.create("/v1/wallets/initialize"))
...
<Tab value="curl">
```bash
curl "/v1/wallets?view=summary" \
  -H "Authorization: Bearer sk_test_..."
```
</Tab>
<Tab value="TypeScript">
```typescript
const response = await fetch("/v1/wallets?view=summary", {
...
```java
HttpRequest request = HttpRequest.newBuilder()
    .uri(URI.create("/v1/wallets?view=summary"))
    .header("Authorization", "Bearer sk_test_...")
    .GET()
    .build();
```
...
<Tab value="curl">
```bash
curl -X POST /v1/wallets/default-wallet \
  -H "Authorization: Bearer sk_test_..." \
  -H "Content-Type: application/json" \
...
<Tab value="TypeScript">
```typescript
await fetch("/v1/wallets/default-wallet", {
  method: "POST",
  headers: {
    "Authorization": "Bearer sk_test_...",
    "Content-Type": "application/json",
...
```java
HttpRequest request = HttpRequest.newBuilder()
    .uri(URI.create("/v1/wallets/default-wallet"))
    .header("Authorization", "Bearer sk_test_...")
    .header("Content-Type", "application/json")
    .POST(HttpRequest.BodyPublishers.ofString("""
        { "walletId": "wal_xyz789" }"""))
