...
For tokens with `requiresAllowlist: true`, the mint endpoints auto-add a fresh destination wallet to the on-chain allowlist (and mirror it into the DB allowlist) when needed. Both `POST /mint` and `POST /allowlist` require `tokens:write`, so any key that can mint can implicitly grow the allowlist. If you want every destination operator-approved before minting, gate `POST /mint` behind your own workflow that pre-calls `POST /allowlist` first, rather than relying on the auto-add behavior.
