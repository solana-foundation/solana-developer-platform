...
<Tab value="TypeScript">
```typescript title="prepare-mint.ts"
const response = await fetch(
...
<Tab value="Java">
```java title="ForceBurnTokens.java"
HttpRequest request = HttpRequest.newBuilder()
...
<Tab value="TypeScript">
```typescript title="refresh-supply.ts"
await fetch(
