...
Fetch the distinct labels in use with `GET /v1/issuance/tokens/{tokenId}/allowlist/labels`, which returns `{ labels: string[]; total: number }` (`total` is the unfiltered entry count) — useful for building a label filter and summary count.
