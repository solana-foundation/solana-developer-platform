...
<Tab value="curl">
```bash
curl -X POST /v1/issuance/tokens/tok_abc123/allowlist \
...
const response = await fetch(
  "/v1/issuance/tokens/tok_abc123/allowlist",
  {
    method: "POST",
    headers: {
...
HttpRequest request = HttpRequest.newBuilder()
    .uri(URI.create("/v1/issuance/tokens/tok_abc123/allowlist"))
...
<Tab value="curl">
```bash
curl /v1/issuance/tokens/tok_abc123/allowlist \
  -H "Authorization: Bearer sk_test_..."
```
</Tab>
<Tab value="TypeScript">
...
```bash
curl -X DELETE \
  /v1/issuance/tokens/tok_abc123/allowlist/alw_abc123 \
  -H "Authorization: Bearer sk_test_..."
```
</Tab>
<Tab value="TypeScript">
...
```java
HttpRequest request = HttpRequest.newBuilder()
    .uri(URI.create("/v1/issuance/tokens/tok_abc123/allowlist/alw_abc123"))
    .header("Authorization", "Bearer sk_test_...")
    .DELETE()
    .build();
```
