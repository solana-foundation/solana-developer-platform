...
All endpoints live under `/v1/earn`. Authenticate with `Authorization: Bearer <SDP_API_KEY>`; the key needs the `earn:read` and `earn:write` permissions. Start in sandbox: new vault deposits are enabled there first.
...
```ts
const SDP_API_URL = "https://api.sdp.solana.com";
...
async function sdpFetch(path: string, init?: RequestInit) {
  ...
  if (!response.ok) {
    throw new Error(result?.error?.message ?? `SDP request failed: ${path} (${response.status})`);
  }
  return result.data;
}
```
...
const { transaction } = await sdpFetch("/v1/earn/external-wallet/deposit-transactions", {
  ...
  body: JSON.stringify({
    ...
    minSharesOut, // slippage floor; required in production
  }),
});
...
On the Treasury surface (your organization's own custody wallets) the wallet itself pays its fees, so keep some SOL on custody wallets that deposit into vaults.
...
- `GET /v1/earn/external-wallet/positions?ownerAddress=…` lists live positions. Page it to completion; a silently short list hides withdrawable money. Read `id` and `withdrawableShares` here to drive withdrawals.
...
const { transaction } = await sdpFetch("/v1/earn/external-wallet/withdrawal-transactions", {
  method: "POST",
  headers: sdpHeaders(),
  body: JSON.stringify({ positionId, shares }), // + feePayer to sponsor it
});
