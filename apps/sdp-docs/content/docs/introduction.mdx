...
Every enforcement action carries an `Idempotency-Key`: reuse the same key when retrying the exact request so a lost response replays the original action instead of submitting it twice.
</Step>
<Step>
...
Batch Transfers are value-moving, so every request carries an `Idempotency-Key`. Reuse the same key when retrying the exact request: a retry after a lost response then replays the original batch instead of creating a second one and paying every recipient twice.
