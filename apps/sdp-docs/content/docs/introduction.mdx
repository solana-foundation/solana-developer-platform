...
Those same extensions are what make compliance enforceable onchain. Freeze individual accounts, pause token activity, seize tokens, and manage allowlists for regulated issuance flows. Selected compliance actions accept reason or memo fields for audit trails. Every enforcement action is one POST on the same token surface.
...
Send stablecoins from custody wallets — single transfers, memo-tagged payments, and batch payouts — and move between fiat and crypto through our integrated ramp partners. One API covers rate estimates, quotes, and settlement across every provider and corridor, with counterparty KYC requirements collected per provider exactly when they are needed. See [Payments](/docs/payments) for transfers and [Ramps](/docs/payments/ramps) for supported providers, currencies, and corridors.
</Step>
<Step>
### Audit Trails
...
| Compliance actions      | Freeze, unfreeze, pause, unpause, seize, and allowlist changes                     |
...
| Counterparties & assets | Counterparty and payout account changes, asset profile changes                     |
</Step>
</Steps>
