...
SDP provides tokenization, custody, payments, and compliance through one stable API in front of interchangeable providers. Operations use project-scoped authentication.
...
const response = await fetch(
  "https://api.solana.com/v1/issuance/tokens/tok_abc123/freeze",
  {
    ...
    headers: {
      Authorization: "Bearer sk_test_...",
      "Content-Type": "application/json",
      "Idempotency-Key": "compliance-freeze-001",
    },
...
curl -X POST https://api.solana.com/v1/issuance/tokens/tok_abc123/freeze \
  -H "Authorization: Bearer sk_test_..." \
  -H "Content-Type: application/json" \
  -H "Idempotency-Key: compliance-freeze-001" \
  -d "$OPERATION_PARAMS"
```
</Tab>
</Tabs>

Every enforcement action carries an `Idempotency-Key`: reuse the same key when retrying the exact request so a lost response replays the original action instead of submitting it twice. Each new action gets a fresh key — a different token or account is a new operation, not a retry.
</Step>
<Step>
...
curl -X POST https://api.solana.com/v1/payments/transfer-batches \
  -H "Authorization: Bearer sk_test_..." \
  -H "Content-Type: application/json" \
  -H "Idempotency-Key: batch-2026-06-30" \
  -d '{
...
Batch Transfers are value-moving, so every request carries an `Idempotency-Key`. Reuse the same key when retrying the exact request: a retry after a lost response then replays the original batch instead of creating a second one and paying every recipient twice. A new batch — different recipients, amounts, or payout date — is a new operation and needs a fresh key; replaying the old key with changed details returns a conflict rather than silently creating the wrong batch.
