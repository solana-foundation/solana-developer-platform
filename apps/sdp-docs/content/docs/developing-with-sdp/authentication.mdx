...
Authenticated API requests carry an API key in the `Authorization` header:
...
Embedded Yield strategy reads, deposit and withdrawal previews, withdrawal-route discovery, and instant deposit or direct-withdrawal builders also support [keyless access](/docs/guides/embedded-yield#without-an-api-key). Those requests have no organization or project identity and cannot submit transactions, build queued request or cancellation actions, or read tenant state.
...
| --- | --- | --- |
| No API key on a keyed-only request | 401 | `UNAUTHORIZED` |
