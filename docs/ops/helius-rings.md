...
tab's From/To pair already expresses), SPL ring spends (SOL-only is this
build's rule — the route schema's mint literal, `requireProtocolSol`, and the
wire gate — not the SDK's), audit reads and grants to further readers
(bring-up's initial grant makes the custody-held config authority the ring's
only reader, so serving decrypted reads or granting a third-party reader needs
a future custody-signed endpoint), and `GET /rings/:name` point reads.
