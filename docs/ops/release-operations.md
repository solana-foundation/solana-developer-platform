...
- **Release artifacts** — Generated by the Release Flow GitHub Action after approval
...
Optional repository variables cap the work performed by one release: `TRANSLATION_LLM_MAX_KEYS` (default `500`), `TRANSLATION_LLM_BATCH_SIZE` (default `50`), and `TRANSLATION_LLM_MAX_RETRIES` (default `2`). The action translates only missing keys, preserves existing catalog values, validates current source keys/placeholders/markup, and commits generated values to the release PR. Stale target keys remain harmless when an English source string is removed; they can be cleaned up separately without blocking releases. If no locale is missing strings, it reports a no-op. Missing provider configuration or invalid generated output fails the workflow instead of producing an incomplete catalog.
