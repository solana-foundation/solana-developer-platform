...
If the database transaction fails after the pending witness is established, or the process exits before promotion, the witness remains pending and all subsequent verification and writes stay locked for incident review. A missing checkpoint for any non-empty ledger, or a pending, malformed, unexpectedly advanced, or forked checkpoint, likewise fails closed. Do not delete, expire, evict, flush, or manually rewrite this key. Configure the Redis service for HA and a policy that cannot evict non-expiring keys; alert on memory pressure and checkpoint mismatch.
...
If Redis contains a pending witness after a failed commit or promotion, preserve it with the database snapshot and verifier output. Compare both the pending `previous` and `next` values with the transaction outcome and independently retained operations record before any approved recovery. If Redis is missing—even when PostgreSQL contains only sequence one—the state is ambiguous and requires the incident procedure above. Never automatically advance, rewind, or reseed a checkpoint from PostgreSQL alone.
