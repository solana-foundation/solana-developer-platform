...
- **Status sets live once in `@sdp/types`, meanings included** — a status vocabulary is a `X_STATUSES as const` tuple with the type derived from it, and every semantic subset or predicate over it (terminal, in-flight, fundable, editable, blocks-next-cycle, transition tables) is a named export beside the tuple (`as const satisfies readonly X[]`, `Record<X, boolean>` + `isFooStatus()`, `Record<Operation, …>`; pattern: `RAMP_TRANSFER_STATUS_TERMINAL` / `isTerminalRampTransferStatus`). Any file outside `packages/sdp-types` that spells status literals to express a meaning is a finding even when typed against the canonical union: `const TERMINAL: PaymentTransferStatus[] = ["canceled", "failed", "expired"]`, `new Set<Status>([...])`, `status === "a" || status === "b"`, `.includes("x")`, `statuses: ["pending", "processing"]`, SQL `IN ('a','b')`. Tests included — a subset re-listed in a test fixture is the same drift. Only migrations and single-status row assignments (`status: "processing"`) are exempt. Fix: name the meaning, export it from `@sdp/types`, import it at every site; gate with a grep for the literals outside `packages/sdp-types`.
...
- **SWR keys via factory** — in `apps/sdp-web`, every `useSWR`/`useSWRMutation` key and global `mutate(...)` key comes from the feature's colocated query-key factory (`payments/payments-query-key.ts`, `custody/custody-query-key.ts`, `issuance/issuance-query-key.ts`, `markets/earn/earn-query-key.ts` — same pattern for new features). An inline string/array key in a component or hook is a finding; name the factory to extend or create. Conditional gating (`cond ? key : null`) stays at the call site.
