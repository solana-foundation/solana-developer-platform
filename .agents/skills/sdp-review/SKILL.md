...
- **Package-first functions** — for every new non-trivial function, ask: does an already-installed package or stdlib implement this? If yes, use it. Adding a new dependency needs genuine justification (ponytail ladder); a few lines of code beat a new dep.
...
- **Transactions all-or-nothing, tenant-scoped** — multiple related DB writes wrapped in ONE `db.transaction` (via `asTransactionalClient`); partial-write sequences are findings. Every query in the change scoped by the authenticated tenant/parent ID — get/update/archive too, not just list.
