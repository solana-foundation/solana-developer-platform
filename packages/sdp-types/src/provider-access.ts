 ...
 *
 * Capability first: an asynchronous next-NAV subscription that cannot encode
 * a floor returns null rather than advertising protection it cannot enforce.
 * Otherwise a provider that refuses an implicit floor requires one in every
 * environment, and production supplies the conservative default when the
 * provider has not declared its own tolerance.
 */
