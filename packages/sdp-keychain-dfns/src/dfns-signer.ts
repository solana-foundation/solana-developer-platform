...
export class DfnsSigner<TAddress extends string = string> implements SolanaSigner<TAddress> {
  ...
  private async fetchWallet(): Promise<DfnsWalletResponse<TAddress>> {
    ...
    return wallet as DfnsWalletResponse<TAddress>;
  }

  private cacheWalletMetadata(wallet: DfnsWalletResponse): void {
  ...
  private getWalletAddress(wallet: DfnsWalletResponse<TAddress>): Address<TAddress> {
    if (!wallet?.address) {
