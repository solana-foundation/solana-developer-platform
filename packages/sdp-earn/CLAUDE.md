...
`supportsVaultWithdraw`, which Kamino implements since PRO-1702 (the batching
work that had kept it withheld landed with the route). The split is not
taxonomy: an exit may legitimately need several transactions (one withdraw
instruction per reserve the vault draws from), which a deposit never does, so
"can build a deposit" must not silently assert "can build a correctly BATCHED
exit" — a future vault provider may ship deposit-only and its exit route
answers 501 until it implements this capability too. The capability answer is
