...
`supportsVaultWithdraw`, and NEITHER implements it yet — for different reasons
worth keeping straight. Kamino CAN build an exit but not a correctly BATCHED
one (a multi-reserve exit may exceed one packet), so the capability is
withheld until the batching lands. Veda can build a correct single-transaction
exit; what is missing is on SDP's side — there is no `POST /vault-withdrawals`
route at all, and Veda's queued exit does not fit the movement model
(`docs/decisions/0003-veda-vault-withdrawals.md`).

The split is not taxonomy either way: "can build a deposit" must not silently
assert "can build an exit SDP can carry". Withholding it says the SDP route
does not exist; it is never a permission gate, since ADR 0002 forbids money-out
inheriting a money-in gate, and it traps nothing — the shares sit in the
organization's own custody wallet and each provider's own surfaces can redeem
them.
...
(`POST /v1/earn/vault-deposits`). Those packages depend on this one, never the
reverse: the hourly catalogue cron must not load a chain SDK it never calls —
and in Veda's case must not need a PRIVATE npm credential to install, either.
