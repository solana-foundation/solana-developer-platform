...
| — (SDP-only) | `PUT /v1/earn/program` also takes `fundingWalletId` (`custody_wallets.id`, migration 0050): the org's own wallet that funds the program and receives withdrawals. Ground never sees it, and no API moves funds from it — funding stays a transfer to the address below. |
