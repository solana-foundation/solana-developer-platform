...
export {
  type AnchorageProvisioningConfig,
...
export { normalizePem } from "./common";
export type { CustodyProvisioningRuntime } from "./runtime";
...
export interface FireblocksProvisioningConfig {
  apiKey?: string;
  apiSecretPem?: string;
  apiBaseUrl?: string;
  assetId?: string;
...
export interface CoinbaseCdpProvisioningConfig {
  apiKeyId?: string;
  apiKeySecret?: string;
  walletSecret?: string;
  apiBaseUrl?: string;
...
export interface UtilaProvisioningConfig {
...
export async function provisionFireblocksVaultAccount(
  ...
  config: FireblocksProvisioningConfig,
  options: ProvisionFireblocksOptions
): Promise<ProvisionFireblocksResult> {
  const apiKey = config.apiKey;
  const apiSecretPem = config.apiSecretPem ? normalizePem(config.apiSecretPem) : undefined;
  ...
  if (!vaultAccountId) {
    ...
    const response = await fireblocksRequest<FireblocksVaultAccountResponse>(runtime, {
      apiBaseUrl,
      apiKey,
      apiSecretPem,
      method: "POST",
  ...
  await fireblocksRequest<void>(runtime, {
    apiBaseUrl,
    apiKey,
    apiSecretPem,
    method: "POST",
  ...
  const addresses = await fetchFireblocksAddressesWithRetry(runtime, {
    apiBaseUrl,
    apiKey,
    apiSecretPem,
    vaultAccountId,
...
export async function provisionCoinbaseCdpAccount(
  runtime: CustodyProvisioningRuntime,
  config: CoinbaseCdpProvisioningConfig,
  ...
  const walletSecret = config.walletSecret;
  ...
  try {
    ...
    try {
      const existingByName = await coinbaseCdpRequest<CoinbaseCdpSolanaAccountResponse>(runtime, {
        method: "GET",
...
export async function provisionParaWallet(
  ...
  const created = await paraRequest<ParaWalletResponse>(runtime, {
    apiBaseUrl,
    apiKey,
    method: "POST",
    path: "/v1/wallets",
  ...
  const readyWallet = await waitForParaWalletReady(runtime, {
    apiBaseUrl,
    apiKey,
    walletId: created.id,
  });
...
export async function provisionTurnkeyPrivateKey(
  runtime: CustodyProvisioningRuntime,
  config: TurnkeyProvisioningConfig,
  ...
  const created = await turnkeyRequest<TurnkeyActivityResponse>(runtime, {
    ...
    body: {
      ...
      timestampMs: runtime.now().toString(),
      organizationId,
      parameters: {
        privateKeys: [
          {
            privateKeyName: buildTurnkeyPrivateKeyName(runtime, options.orgSlug || options.orgId),
            curve: "CURVE_ED25519",
            privateKeyTags: [],
...
export async function provisionUtilaWallet(
  runtime: CustodyProvisioningRuntime,
  config: UtilaProvisioningConfig,
  options: ProvisionUtilaOptions
): Promise<ProvisionUtilaResult> {
  const serviceAccountEmail = config.serviceAccountEmail;
  const serviceAccountPrivateKeyPem = config.serviceAccountPrivateKeyPem;
  const configuredVaultId = config.vaultId;
  const network = config.network ?? DEFAULT_UTILA_NETWORK;
...
async function fetchFireblocksAddressesWithRetry(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    const response = await fireblocksRequest<VaultAddressesResponse>(runtime, {
...
async function fireblocksRequest<T>(
  runtime: CustodyProvisioningRuntime,
  params: FireblocksRequestParams
): Promise<T> {
  const bodyStr = params.body ? JSON.stringify(params.body) : "";
  const token = await createFireblocksJwt(
    runtime,
    params.apiKey,
    params.apiSecretPem,
    params.uri,
  ...
  const response = await runtime.fetch(`${params.apiBaseUrl}${params.uri}`, {
...
async function createFireblocksJwt(
  ...
  const bodyHash = await sha256Hex(runtime, body);
...
interface PrivyRequestParams {
  ...
  idempotencyKey?: string;
  storedCredentialRequest?: boolean;
  method: "GET" | "POST";
  path: string;
  body?: unknown;
}
...
async function privyRequest<T>(
  ...
  try {
    const response = await runtime.fetch(`${params.apiBaseUrl}${params.path}`, {
      ...
      signal: params.storedCredentialRequest
        ? AbortSignal.timeout(PRIVY_REQUEST_TIMEOUT_MS)
        : undefined,
    });
    ...
    if (!response.ok) {
      const errorText = await readErrorResponseText(response);
      const code =
        response.status === 401 && params.storedCredentialRequest
          ? "PROVIDER_CREDENTIAL_INVALID"
          : response.status === 404 && params.allowNotFound
            ? "NOT_FOUND"
            : response.status >= 500 && params.storedCredentialRequest
              ? "NETWORK_ERROR"
              : "PROVIDER_NOT_CONFIGURED";
