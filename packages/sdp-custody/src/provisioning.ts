...
import { deleteAnchorageWallet, provisionAnchorageWallet } from "./provisioning.anchorage";
import {
...
export interface ProvisionFireblocksConfig {
  orgId: string;
  orgSlug: string;
  assetId?: string;
  apiBaseUrl?: string;
...
export interface ProvisionPrivyConfig {
  appId?: string;
  appSecret?: string;
  walletId?: string;
  apiBaseUrl?: string;
...
export interface ProvisionCoinbaseCdpConfig {
  orgId: string;
  orgSlug: string;
  apiBaseUrl?: string;
...
export interface ProvisionTurnkeyConfig {
  orgId: string;
  orgSlug: string;
  privateKeyId?: string;
  apiBaseUrl?: string;
...
export interface ProvisionParaConfig {
  orgId: string;
  orgSlug: string;
  projectId?: string;
  walletId?: string;
...
export interface ProvisionUtilaConfig {
...
export async function provisionFireblocksVaultAccount(
  options: ProvisionFireblocksConfig
): Promise<ProvisionFireblocksResult> {
  const apiKey = options.apiKey;
  const apiSecretPem = options.apiSecretPem ? normalizePem(options.apiSecretPem) : undefined;
  ...
  const apiBaseUrl = options.apiBaseUrl ?? DEFAULT_FIREBLOCKS_API_BASE_URL;
  const assetId = options.assetId ?? DEFAULT_FIREBLOCKS_ASSET_ID;
...
export async function provisionPrivyWallet(
  ...
  const apiBaseUrl = options.apiBaseUrl ?? DEFAULT_PRIVY_API_BASE_URL;
...
export async function provisionCoinbaseCdpAccount(
  ...
  const apiBaseUrl = options.apiBaseUrl ?? DEFAULT_COINBASE_CDP_API_BASE_URL;
  const network = options.network ?? DEFAULT_COINBASE_CDP_NETWORK;
  ...
  const name = buildCoinbaseCdpAccountName(options.orgSlug || options.orgId, options.accountScope);
...
export async function provisionParaWallet(
  ...
  const apiBaseUrl = options.apiBaseUrl ?? DEFAULT_PARA_API_BASE_URL;
...
export async function provisionTurnkeyPrivateKey(
  options: ProvisionTurnkeyConfig
): Promise<ProvisionTurnkeyResult> {
  const apiPublicKey = options.apiPublicKey;
  const apiPrivateKey = options.apiPrivateKey;
  ...
  const apiBaseUrl = options.apiBaseUrl ?? DEFAULT_TURNKEY_API_BASE_URL;
...
export async function provisionUtilaWallet(
  options: ProvisionUtilaConfig
): Promise<ProvisionUtilaResult> {
  const serviceAccountEmail = options.serviceAccountEmail;
  const serviceAccountPrivateKeyPem = options.serviceAccountPrivateKeyPem;
  const configuredVaultId = options.vaultId;
  const network = options.network ?? DEFAULT_UTILA_NETWORK;
  ...
  const apiBaseUrl = normalizeUtilaApiBaseUrl(options.apiBaseUrl);
...
export interface CustodyProvisioning {
  provisionFireblocksVaultAccount: typeof provisionFireblocksVaultAccount;
  provisionPrivyWallet: typeof provisionPrivyWallet;
  provisionCoinbaseCdpAccount: typeof provisionCoinbaseCdpAccount;
  provisionParaWallet: typeof provisionParaWallet;
  provisionTurnkeyPrivateKey: typeof provisionTurnkeyPrivateKey;
  provisionUtilaWallet: typeof provisionUtilaWallet;
  provisionAnchorageWallet: typeof provisionAnchorageWallet;
  deleteAnchorageWallet: typeof deleteAnchorageWallet;
}

export const custodyProvisioning = {
...
} satisfies CustodyProvisioning;

export { normalizePem } from "./provisioning.common";
export { deleteAnchorageWallet, provisionAnchorageWallet };
