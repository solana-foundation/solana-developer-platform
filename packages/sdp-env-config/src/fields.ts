...
export const FIELDS: EnvField[] = [
  ...
  {
    key: "HELIUS_RINGS_KEY_AUTHORITY",
    section: "advanced",
    kind: "select",
    label: "Helius Rings key authority",
    defaultValue: "database",
    options: [
      { value: "database", label: "Database (random keys, encrypted at rest)" },
      { value: "deterministic", label: "Deterministic (public seed, development only)" },
    ],
    help: "Where new Rings wallets get their shielded keys. 'database' requires the Rings key encryption key. 'deterministic' derives from a seed committed to the source and is rejected outside development. Existing wallets keep the authority they were provisioned under, so changing this only affects wallets created afterwards.",
  },
  {
    key: "RINGS_KEY_ENCRYPTION_KEY",
    section: "secrets",
    kind: "secret",
    secretEncoding: "base64",
    label: "Rings key encryption key",
    help: "Base64-encoded 256-bit key for encrypting Helius Rings shielded key material. Separate from the custody key so compromising one cannot expose the other. Losing it permanently orphans every note held by a database-authority wallet.",
  },
  {
    key: "RINGS_KEY_KMS_KEY_NAME",
    section: "secrets",
    kind: "text",
