...
describe("evaluateAmountRule", () => {
  it("reviews a rule that names no assets instead of bounding every asset", () => {
    assert.partialDeepStrictEqual(evaluateAmountRule({ kind: "amount", max: "100" }, operation), {
      decision: "review",
      reason: "Amount rule has no assets.",
    });
  });
  ...
  it("abstains when the operation carries no amount", () => {
    assert.equal(
      evaluateAmountRule(
        { kind: "amount", asset: "USDC", max: "100" },
  ...
  it("reviews a rule with no bounds", () => {
    assert.partialDeepStrictEqual(
      evaluateAmountRule({ kind: "amount", asset: "USDC" }, operation),
      {
        decision: "review",
  ...
  it("reviews an empty-string bound instead of treating it as absent", () => {
    assert.partialDeepStrictEqual(
      evaluateAmountRule({ kind: "amount", asset: "USDC", min: "" }, operation),
      {
        decision: "review",
  ...
  it("reviews an invalid operation amount", () => {
    assert.partialDeepStrictEqual(
      evaluateAmountRule(
        { kind: "amount", asset: "USDC", max: "100" },
        { ...operation, amount: "not-a-decimal" }
      ),
  ...
  it("denies below the minimum and above the maximum", () => {
    const belowMin = evaluateAmountRule({ kind: "amount", asset: "USDC", min: "200" }, operation);
    const aboveMax = evaluateAmountRule({ kind: "amount", asset: "USDC", max: "100" }, operation);
  ...
  it("allows within bounds", () => {
    const evaluation = evaluateAmountRule(
      { kind: "amount", asset: "USDC", min: "100", max: "200" },
      operation
    );
