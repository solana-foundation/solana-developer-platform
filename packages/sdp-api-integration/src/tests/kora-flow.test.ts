...
describe("Kora Fee Payment (Live Smoke)", () => {
  ...
  it("submits a Privy signer-check memo through Kora signAndSend", {
    ...
    // The memo is server-generated now, so the request carries none and the
    // response is asserted against the shape the API mints.
    const serverMemoPattern =
      /^SDP signer check [0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/;
    ...
    try {
      ...
      expect(signerCheck.memo).toMatch(serverMemoPattern);
