...
describe.skipIf(!SOLANA_CONFIGURED || !RUN_INTEGRATION_TESTS)("Issuance Endpoint Coverage", () => {
  ...
  it("covers templates, token CRUD, and allowlist endpoints", async () => {
    ...
    // biome-ignore lint/nursery/noSecrets: Query params used for pagination, not a secret.
  ...
  it(
    "covers deploy/mint/burn/seize/force-burn/authority prepare endpoints and pending history",
    { timeout: 240000 },
    async () => {
      ...
      const forceBurnPrepareRes = await request(
        `/v1/issuance/tokens/${tokenId}/force-burn/prepare`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      const authorityPrepareRes = await request(
        `/v1/issuance/tokens/${tokenId}/authority/prepare`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  it(
    "covers execute endpoints, frozen-account endpoints, confirmed history, and supply refresh",
    { timeout: 300000 },
    async () => {
      ...
      const frozenListRes = await request(
        `/v1/issuance/tokens/${tokenId}/frozen?page=1&pageSize=10`
      );
