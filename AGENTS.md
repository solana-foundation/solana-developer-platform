...
- `earn` is held out of every public surface until launch (PRO-2038): `EARN_PUBLIC_SURFACE_PUBLISHED` in `apps/sdp-api/src/openapi/spec.ts` drops the family from the public OpenAPI document, and the API reference, Postman collection, playground catalog and AI files regenerate from it. It also holds the unified transaction list at the document boundary (`apps/sdp-api/src/openapi/paths/transactions.ts`, via `apps/sdp-api/src/routes/transactions/publication.ts`): while it is false, the module selector omits held-back modules and the published response union gains a module-agnostic variant, so an unfiltered read's held-back rows stay parseable for public-contract clients without the document naming them. The runtime is unchanged. Flipping it back is a PRO-1872 security sign-off PR. Explicitly requested modules and the internal document are unchanged.
